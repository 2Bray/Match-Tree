...
public class TileControllerScript : MonoBehaviour
{
    ...
    private BoardManagerScript board;
    private SpriteRenderer render;

    private static readonly Color selectedColor = new Color(0.5f, 0.5f, 0.5f);
    private static readonly Color normalColor = Color.white;

    private static TileControllerScript previousSelected = null;
    ...
    private GameFlowManagerScript game;
    ...
    private void Awake()
    {
        game = GameFlowManagerScript.Instance;
        board = BoardManagerScript.Instance;
    ...
    private void OnMouseDown()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    TileControllerScript otherTile = previousSelected;
                    ...
                    SwapTile(otherTile, () => {
                        ...
                        {
                            SoundManagerScript.Instance.PlayWrong();
                            SwapTile(otherTile);
                        }
                    });
                }
    ...
    public void SwapTile(TileControllerScript otherTile, System.Action onCompleted = null)
    {
    ...
    #region Adjacent
    private TileControllerScript GetAdjacent(Vector2 castDir)
    {
        RaycastHit2D hit = Physics2D.Raycast(transform.position, castDir, render.size.x);

        if (hit)
        {
            return hit.collider.GetComponent<TileControllerScript>();
        }

        return null;
    }

    public List<TileControllerScript> GetAllAdjacentTiles()
    {
        List<TileControllerScript> adjacentTiles = new List<TileControllerScript>();
    ...
    #region Check Match
    private List<TileControllerScript> GetMatch(Vector2 castDir)
    {
        ...
        while (hit)
        {
            TileControllerScript otherTile = hit.collider.GetComponent<TileControllerScript>();
    ...
    private List<TileControllerScript> GetOneLineMatch(Vector2[] paths)
    {
        List<TileControllerScript> matchingTiles = new List<TileControllerScript>();
    ...
    public List<TileControllerScript> GetAllMatches()
    {
        ...
        List<TileControllerScript> horizontalMatchingTiles = GetOneLineMatch(new Vector2[2] { Vector2.up, Vector2.down });
        List<TileControllerScript> verticalMatchingTiles = GetOneLineMatch(new Vector2[2] { Vector2.left, Vector2.right });
